...
const int OUTPUTTRANNY = 5; //Output, pin no. matches
...
//  pinMode(BIASPOT, INPUT);
  ...
  pinMode(OUTPUTTRANNY, OUTPUT);
  digitalWrite(OUTPUTTRANNY, LOW);
...
void loop() {
  ...
  int bias = readAvg(5, BIASPOT); //5 averages of the bias pot
...
/*  loop that writes led status on every iteration
  if(temp > bias){ //Temp is too high, kick on heater and led
    ...
    digitalWrite(OUTPUTTRANNY, HIGH); 
...
*/
  ...
  delay(250); //Delay 1/4 sec, to prevent hysterisis. Possibly increase this to >1 sec, depending on relay behavior.
}
